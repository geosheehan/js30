...
const sixteenthCenturyInventors = inventors.filter((inventor) =>
   1500 <= inventor.year && inventor.year < 1600);
...
const yearsLived = inventors.reduce((years, inventor) => years += getAge(inventor), 0);
console.log(yearsLived);

// 5. Sort the inventors by years lived
const sortedYearsLived = inventors.sort((x, y) => getAge(y) - getAge(x));
...
const sortedLastName = people.sort((lhs, rhs) => {
   const leftLast = lhs.split(',')[0];
   const rightLast = rhs.split(',')[0];
...
const vehicle = data.reduce((obj, x) => {
   if (x in obj) {
      obj[x]++;
   }
   else {
